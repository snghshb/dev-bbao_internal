...
<tr>
<td>
<form class="form-horizontal" action="studentGradesAddNew.php" name="studentGradesAddNew" method="post" enctype="multipart/form-data" >
	<div class="form-group">
		<label class="control-label col-sm-2" for="UIN">Select Student</label>
		<div class="col-sm-10">
			<select class="form-control" id="UIN" name="UIN">
<?php
$sql = "SELECT UIN, studentLastName, studentFirstName from bbaonline.bbao_students_info WHERE enrollmentYear = 2016 AND enrollmentTerm = 'FA' AND enrollmentType = 0 ORDER BY `enrollmentYear`, `UIN`";
$result = mysqli_query($conn,$sql);
while($row = $result->fetch_assoc()) {
	echo "<option value='".$row['UIN']."'>".$row['studentLastName'].", ".$row['studentFirstName']." (".$row['UIN'].")</option>";
}
?>
			</select>
		</div>
...
?>
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<select class="form-control" id="grade" name="grade">
				<option value='N'>NA/None/Pursuing</option>
	...
	<div class="form-group">        
		<div class="col-sm-offset-2 col-sm-10">
			<button type="submit" class="btn btn-default">Add Grades</button>
		</div>
    </div>
	<div>
